...
//    public boolean isUpgradeValid() throws NotEnoughXPException, NoMoreUpgradeException, NotEnoughRequiredAbilitiesException
//    {
...
//    }
    ...
    public boolean isCastValid() throws AbilityNotAcquieredException, NotStrongEnoughException, AbilityCooldownException // LOL
    {
        if (currentUpgradeNum == 0)
        {
//            System.out.println("You have not yet acquired this ability");
            throw new AbilityNotAcquieredException("You have not yet acquired this ability");
//            return false;
        }
...
//                    System.out.println("you don't have enough " + attributeNameParts[1]);
                    throw new NotStrongEnoughException("you don't have enough " + attributeNameParts[1]);
//                    return false;
                }
            }
        }
...
//            System.out.println("Your desired ability is still in cooldown");
            throw new AbilityCooldownException("Your desired ability is still in cooldown");
//            return false;
        }
        ...
        for (String attribute : formulas.keySet())
        {
            ...
            if (!attributeNameParts[0].equals("cost"))
            {
                for (Warrior target : targets)
                {
                    ...
                    {
                        System.out.println(hero.getName() + " " + successMessage + " " + target.getName() + "'s " + attribute + " " + formulas.get(attribute).get(currentUpgradeNum - 1).parseFormula(hero.getData()));
                    } else
                    {
                        System.out.println(hero.getName() + " " + successMessage + " " + target.getName() + "'s " + attribute + " " + -1 * formulas.get(attribute).get(currentUpgradeNum - 1).parseFormula(hero.getData()));
    ...
    public ArrayList<String> getFullAbilitySuccessMessage(ArrayList<String> targetNames)
    {
        ...
        for (String attribute : formulas.keySet())
        {
            ...
            if (!attributeNameParts[0].equals("cost"))
            {
                for (String target : targetNames)
                {
                    if (formulas.get(attribute).get(currentUpgradeNum - 1).parseFormula(hero.getData()) > 0)
                    {
//                        System.out.println(hero.getName() + " " + successMessage + " " + target + "'s " + attribute + " " + formulas.get(attribute).get(currentUpgradeNum - 1).parseFormula(hero.getData()));
                        result.add(hero.getName() + " " + successMessage + " " + target + "'s " + attribute + " " + formulas.get(attribute).get(currentUpgradeNum - 1).parseFormula(hero.getData()));
                    } else
                    {
//                        System.out.println(hero.getName() + " " + successMessage + " " + target + "'s " + attribute + " " + -1 * formulas.get(attribute).get(currentUpgradeNum - 1).parseFormula(hero.getData()));
                        result.add(hero.getName() + " " + successMessage + " " + target + "'s " + attribute + " " + -1 * formulas.get(attribute).get(currentUpgradeNum - 1).parseFormula(hero.getData()));
                    }
                }
            }
        }
